...
{
    ESP_LOGI(TAG_BT, "BLE Host Task Started");
...
void bt_hid_main() {
    ...
    ESP_LOGI(TAG_BT, "setting hid gap, mode:%d", HID_DEV_MODE);
    ESP_LOGI(TAG_BT, "This is a test");
    ...
    ESP_LOGI(TAG_BT, "setting ble device");
    ESP_ERROR_CHECK(
    ...
    if (ret) {
        ESP_LOGE(TAG_BT, "esp_nimble_enable failed: %d", ret);
    }
